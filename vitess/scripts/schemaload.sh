...
if [ ! -f schema_run ]; then
  ...
  if [ "$external_db" = "0" ]; then
    for schema_file in $schema_files; do
      ...
      vtctldclient --server vtctld:$GRPC_PORT ApplySchema --sql "$(cat /script/sql/${schema_file})" $KEYSPACE || true
    done
  fi
  ...
  vtctldclient --server vtctld:$GRPC_PORT ApplyVSchema --vschema "$(cat /script/configs/${vschema_file})" $KEYSPACE
  ...
  if [ -n "$load_file" ]; then
    ...
    sleep 60
    mysql --port=15306 --host=vtgate < /script/configs/$load_file
  fi
