...
nc -z -v -w 5 127.0.0.1 9094 2>&1 && echo "${GREEN}✓ Successfully connected to Kafka02 at 127.0.0.1:9094${NC}" || echo "${RED}✗ Failed to connect to Kafka02 at 127.0.0.1:9094${NC}"

echo "\n${YELLOW}Application Configuration:${NC}"
echo "External applications should use these bootstrap servers:"
echo "${GREEN}bootstrap.servers=127.0.0.1:9092,127.0.0.1:9093,127.0.0.1:9094${NC}"
